...
- EEPROM (Electrically Erasable Programmable read only memory) 
  - 用途和特性
    - EEPROM 是一种**带电可擦可编程只读存储器**，掉电后数据不丢失。可以擦除已有信息并重新编程，广泛用于即插即用设备，如 U 盘。闪存都属于他
    - 非易失性存储，适合嵌入式系统。
    - 用于存储用户设定、设备参数等。
  ...
  - 静态RAM (SRAM) 和动态RAM (DRAM) 比较
    ...
    - 主存和缓存的功能与使用
      ...
      - 主存 vs 缓存 (比较)
        ...
        |--------|--------------|----------------|
        | 速度     | 较慢          | 非常快                         |
        | 目的     | 存储程序和数据 | 加速 CPU 访问数据              |
        | 访问方式 | 由 CPU 或其他硬件直接访问 | CPU 自动管理缓存内容 |
        | 成本     | 较低                     | 较高               |
...
- 基本编程概念
  - 变量类型
    - 整数 Integer、字符 Character、字符串 String、实数 Real（小数在这里）
...
- 伪代码找出bug (逻辑)
  ...
  1. 设计阶段: 需求不对导致设计不对， 需求对设计不对. 和客户确认好用流程图
  2. 编码阶段: 程序的bug 没有遵循设计要求 > 代码审查、单元测试确保代码符合设计文档，防止偏离设计。
  3. 测试阶段: 输入超出范围
